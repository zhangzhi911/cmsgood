...
<body>
	...
	<form id="form1" enctype="application/x-www-form-urlencoded" > 
		...
		<input type="hidden" name="id" value="${upart.id}">
		<label for="title">文章标题:</label>
		<input type="text" class="form-control" name="title" id="title" value="${upart.title}">
		</div>
		
		<div class="form-group">
		<textarea name="content1" cols="100" rows="8" style="width:825px;height:225px;visibility:hidden;"><%=htmlspecialchars(htmlData)%>${upart.content}</textarea>
		</div>		
		...
		<div class="form-group"> <!-- 图片 -->
			标题图片:<input type="file" name="file" value="${upart.picture}" class="form-control">		
		</div>
